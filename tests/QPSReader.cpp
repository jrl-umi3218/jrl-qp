...
#define THROW(message, context) { std::stringstream msg; \
                                  msg << message << context << "\n"; \
...
namespace
{
  ...
  QPSReader::RowType rowType(char c)
  {
    ...
    {
    case 'e': return QPSReader::RowType::E;
    case 'l': return QPSReader::RowType::L;
    case 'g': return QPSReader::RowType::G;
    case 'n': return QPSReader::RowType::N;
    default: return QPSReader::RowType::UNKNOWN;
    }
  }

  QPSReader::BndType bndType(const std::string s)
  {
    if (s == "LO") return QPSReader::BndType::LO;
    else if (s == "UP") return QPSReader::BndType::UP;
    else if (s == "FX") return QPSReader::BndType::FX;
    else if (s == "FR") return QPSReader::BndType::FR;
    else if (s == "MI") return QPSReader::BndType::MI;
    else if (s == "PL") return QPSReader::BndType::PL;
    else return QPSReader::BndType::UNKNOWN;
  }
...
namespace jrlqp::test
{
  ...
  std::pair<QPProblem<>, ProblemProperties> QPSReader::read(const std::string& filename)
  {
    ...
    while (std::getline(file, line))
    {
      ...
      if (lt == LineType::OTHER || lt == LineType::EMPTY)
        continue;
      ...
      else
      {
        context.section = lt;
        if (lt == LineType::NAME)
          problemName = parseName(line, context);
      }
    }
    ...
    for (const auto& x : xVal)
    {
      ...
      switch (x.second)
      {
      case BndType::LO: qp.xl[i] = v; break;
      case BndType::UP: qp.xu[i] = v; break;
      case BndType::FX: qp.xl[i] = qp.xu[i] = v; properties.hasFixedVariables = true;  break;
      case BndType::FR: qp.xl[i] = -bigBnd; qp.xu[i] = bigBnd; break;
      case BndType::MI: qp.xl[i] = -bigBnd; break;
      case BndType::PL: qp.xu[i] = bigBnd; break;
      default: assert(false); break;
      }
    }
  ...
  void QPSReader::processLine(const std::string& line, LineType type)
  {
    ...
    {
    case LineType::NAME: THROW("We shouldn't be in a NAME section ", context);
    case LineType::ROWS: readRow(line); break;
    case LineType::COLUMNS: readColumn(line); break;
    case LineType::RHS: readRHS(line); break;
    case LineType::RANGES: readRanges(line); break;
    case LineType::BOUNDS: readBounds(line); break;
    case LineType::QUADOBJ: readQuadObj(line); break;
    case LineType::ENDATA: THROW("We shouldn't be in a ENDATA section ", context);
    default:
      break;
    }
  }
  ...
  void QPSReader::readRow(const std::string& line)
  {
    ...
    if (mapRow.find(name) != mapRow.end())
      THROW("Duplicate row name", context);

    if (type == RowType::UNKNOWN) THROW("Unknown row type ", context)
  ...
  void QPSReader::readColumn(const std::string& line)
  {
    ...
    if (val2)
      addValueToColumn(cIdx, val2->name, val2->value);
  }
  ...
  void QPSReader::readRHS(const std::string& line)
  {
    ...
    if (val2)
      addValueToRHS(val2->name, val2->value);
  }
  ...
  void QPSReader::readRanges(const std::string& line)
  {
    ...
    if (val2)
      addValueToRanges(val2->name, val2->value);
  }
