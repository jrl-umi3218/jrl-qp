...
namespace jrlqp::test
{
  ...
  {
    characs.check();
    auto [nVar, nObj, nEq, nIneq, rankObj, nSharedRank, nStrongActIneq, nWeakActIneq,
      nStrongActBounds, nWeakActBounds, bounds, doubleSidedIneq, strictlyFeasible] = characs;
    ...
    else if (colsTot > nVar)
    {
      ...
      MatrixXd M(nVar, nObj + nstrong); M << pb.A.transpose(), Ca.transpose();
    ...
    else
    {
      ...
      MatrixXd M(nObj + nstrong, nVar); M << pb.A, Ca;
      ColPivHouseholderQR<MatrixXd> qr(M);
  ...
  {
    assert(nVar_ >= 0 && nObj_ >= 0 && nEq_ >= 0 && nIneq_ >= 0 && rankObj_ >= 0
      && nStrongActIneq_ >= 0 && nWeakActIneq_ >= 0 && nStrongActBounds_ >= 0 && nWeakActBounds_ >= 0);
    assert(nVar_ >= nObj_);
    assert(nVar_ >= nEq_);
    assert(nStrongActIneq_ + nWeakActIneq_ <= nIneq_);
    assert((bounds_ && nStrongActBounds_ + nWeakActBounds_ <= nVar_) || (!bounds_ && nStrongActBounds_ == 0 && nWeakActBounds_ == 0));
