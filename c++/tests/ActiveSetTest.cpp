...
void checkActivation(const internal::ActiveSet& as, const std::vector<bool>& act)
{
  ...
  for (int i = 0; i < as.nbAll(); ++i)
    FAST_CHECK_EQ(as.isActive(i), act[i]);
}

void checkStatus(const internal::ActiveSet& as, const std::vector<ActivationStatus>& status)
{
  for (int i = 0; i < as.nbAll(); ++i)
    FAST_CHECK_EQ(as.activationStatus(i), status[i]);
}

void checkActiveIdx(const internal::ActiveSet& as, const std::vector<int>& actIdx)
{
  FAST_CHECK_EQ(as.nbActiveCstr(), actIdx.size());
  for (int i = 0; i < as.nbActiveCstr(); ++i)
    FAST_CHECK_EQ(as[i], actIdx[i]);
}

void checkNb(const internal::ActiveSet& as, int nbCstr, int nbEq, int nbIneq, int nbLIneq,
  int nbUIneq, int nbBnd, int nbLBnd, int nbUBnd)
{
